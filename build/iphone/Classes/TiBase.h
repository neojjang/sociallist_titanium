...
// in simulator we redefine to format for Handuplist Developer console
...
{\
	NSString * exceptionName = [@"org.handuplist." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.handuplist";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIHanduplistNativeItemNone = -1, 
	UIHanduplistNativeItemSpinner = -2,
	UIHanduplistNativeItemProgressBar = -3,
	
	UIHanduplistNativeItemSlider = -4,
	UIHanduplistNativeItemSwitch = -5,
	UIHanduplistNativeItemMultiButton = -6,
	UIHanduplistNativeItemSegmented = -7,
	
	UIHanduplistNativeItemTextView = -8,
	UIHanduplistNativeItemTextField = -9,
	UIHanduplistNativeItemSearchBar = -10,
	
	UIHanduplistNativeItemPicker = -11,
	UIHanduplistNativeItemDatePicker = -12,
	
	UIHanduplistNativeItemInfoLight = -13,
	UIHanduplistNativeItemInfoDark = -14,
	
	UIHanduplistNativeItemDisclosure = -15,
	
	UIHanduplistNativeItemContactAdd = -16
};
